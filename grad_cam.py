...
def visualize_cam(mask, img):
    ...
    """
    heatmap = cv2.applyColorMap(np.uint8(255 * mask.squeeze()),
                                cv2.COLORMAP_JET)
...
def denormalize(tensor, mean, std):
    ...
    mean = torch.FloatTensor(mean).view(1, 3, 1,
                                        1).expand_as(tensor).to(tensor.device)
    std = torch.FloatTensor(std).view(1, 3, 1,
                                      1).expand_as(tensor).to(tensor.device)
...
class GradCAM(object):
    ...
    def forward(self, input, class_idx=None, retain_graph=False):
        ...
        saliency_map = F.upsample(saliency_map,
                                  size=(h, w),
                                  mode='bilinear',
                                  align_corners=False)
